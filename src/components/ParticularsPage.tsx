...
const DRAFT_BILL_STORAGE_KEY = 'varun_draft_bill';
...
const getSavedDraft = (): DraftBillState => {
  try {
    const raw = localStorage.getItem(DRAFT_BILL_STORAGE_KEY) || localStorage.getItem('dheeksha_draft_bill');
    if (raw) {
...
export const ParticularsPage: FC<ParticularsPageProps> = ({ initialCustomerName }) => {
  ...
  const [company, setCompany] = useState<string>(() => {
    return storeSettings.companyName || 'Varun Trade';
  });
  ...
  useEffect(() => {
    const handleSettingsUpdate = () => {
      ...
      setCompany(updated.companyName || 'Varun Trade');
  ...
  const handleSaveBill = async (andPrint: boolean = false) => {
    ...
    try {
      ...
      localStorage.removeItem('varun_draft_bill');
      localStorage.removeItem('dheeksha_draft_bill');
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Grid container spacing={1.5}>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <Typography sx={{ fontSize: '12.5px', fontWeight: 700, color: '#786C58', mb: 0.6 }}>
                    Phone (Optional)
                  </Typography>
                  <TextField
                    fullWidth
