...
import defaultProjectLogo from '../assets/varun-traders.png';
...
export const getStoredSettings = (): CompanySettings => {
  try {
    const saved = localStorage.getItem('varun_app_settings') || localStorage.getItem('dheeksha_app_settings');
    if (saved) {
      const parsed = JSON.parse(saved);
      if (parsed.companyName === 'Dheeksha Trade' || parsed.companyName === 'Dheeksha Trade Corporation') {
        parsed.companyName = 'Varun Trade';
      }
      if (!parsed.logoUrl) {
...
export const SettingsPage: React.FC = () => {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (data && typeof data === 'object') {
          ...
          window.dispatchEvent(new Event('varun_settings_updated'));
  ...
  const handleSave = async () => {
    try {
      ...
      localStorage.setItem('varun_app_settings', JSON.stringify(settings));
      ...
      if (data && typeof data === 'object' && (data.companyName !== undefined || data._id)) {
        ...
        localStorage.setItem('varun_app_settings', JSON.stringify(syncedSettings));
  ...
  const handleResetToDefault = async () => {
    if (window.confirm('Reset company profile details to default values?')) {
      ...
      localStorage.setItem('varun_app_settings', JSON.stringify(DEFAULT_COMPANY_SETTINGS));
