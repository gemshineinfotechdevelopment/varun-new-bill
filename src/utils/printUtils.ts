...
export const generateBillHtml = (bill: BillPrintData): string => {
  ...
  if (storeSettings.address && storeSettings.address.trim()) {
    addressParts.push(storeSettings.address.trim());
  }
  const cityState = [storeSettings.city, storeSettings.state].filter(Boolean).join(', ');
  if (cityState) {
    addressParts.push(cityState);
  }
  if (storeSettings.pincode && storeSettings.pincode.trim()) {
    addressParts.push(`PIN: ${storeSettings.pincode.trim()}`);
  }
  const fullAddressLine = addressParts.length > 0 ? addressParts.join(' - ') : (storeSettings.city || 'Sivakasi');

  // Contact line (Phone, WhatsApp, Email)
  const contactParts: string[] = [];
  if (storeSettings.phone && storeSettings.phone.trim()) {
    contactParts.push(`Mobile: ${storeSettings.phone.trim()}`);
  }
  if (storeSettings.whatsapp && storeSettings.whatsapp.trim() && storeSettings.whatsapp.trim() !== storeSettings.phone?.trim()) {
    contactParts.push(`WhatsApp: ${storeSettings.whatsapp.trim()}`);
  }
...
<body>
  <div class="bill-box-container">
    ...
    <div class="top-header">
      ${
        `<div style="margin-bottom:4px;"><img src="${storeSettings.logoUrl || '/varun-traders.png'}" alt="Company Logo" style="max-height:48px; max-width:160px; object-fit:contain;" /></div>`
      }
...
const triggerBrowserPrint = (htmlContent: string) => {
  let iframe = (document.getElementById('varun-print-iframe') || document.getElementById('dheeksha-print-iframe')) as HTMLIFrameElement | null;
  if (!iframe) {
